...
getFeatureRanges <- function(gtf,
    ...
    if (length(gtf) != 1L || !is.character(gtf) || !file.exists(gtf)) {
    ...
    if ("intron" %in% featureType) {
        if (length(intronType) != 1L || !is.character(intronType) ||
            !(intronType %in% c("separate", "collapse"))) {
            stop("'intronType' must be either 'separate' or 'collapse'")
        }
        if (!is.numeric(flankLength) || length(flankLength) != 1L ||
            flankLength < 0L) {
