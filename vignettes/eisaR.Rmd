...
```{r loadcounts}
cntEx <- readRDS(system.file("extdata", "Fig3abc_GSE33252_rawcounts_exonic.rds", package = "eisaR"))
cntIn <- readRDS(system.file("extdata", "Fig3abc_GSE33252_rawcounts_intronic.rds", package = "eisaR"))
```
...
exactly as described in [@eisa]. This may be preferrable to understand each
individual step and be able to access intermediate results.

The results obtained in this way are essentially the same to what you get with
...
design <- model.matrix(~ fRegion * fCell)
