...
They are provided in the form of `TxDb` objects, e.g. via packages such as `r Biocpkg("TxDb.Mmusculus.UCSC.mm10.knownGene")`.
You can see available annotations using the following code:  

```{r availableOnline, eval=FALSE}
pkgs <- BiocManager::available()
grep("TxDb", pkgs, value = TRUE)
```
...
## Two alternative implementations of EISA
Using the argument `method`, you can choose between two alternative implementations
of EISA. `method="published"` will use an implementation that corresponds to the
algorithm described in [@eisa]. `method="new"` will use more of the functionality
provided by `edgeR` (e.g. for normalization and effect size calculation), summarized
in the table below:  

| Aspect:           | `method="published"`                        | `method="new"`                                   |
|-------------------|---------------------------------------------|--------------------------------------------------|
| normalization     | linear scaling to the mean library size     | `edgeR::calcNormFactors`                         |
| gene selection    | log2(x + 8) > 5 (in both exons and introns) | cpm > 5 in at least 2 samples (exons or introns) |
| effect size       | log2((x + 8)/(y + 8))                       | `edgeR::predFC`                                  |
| model fit         | `edgeR::glmFit`                             | `edgeR::glmQLFit`                                |
| significance test | `edgeR::glmLRT`                             | `edgeR::glmQLFTest`                              |

While both methods yield highly similar results, `method="new"` is often less
stringent when selecting quantifyable genes, but more stringent when calling
significant changes (especially with low numbers of replicates).

Here is an illustration of how the results differ between the two:  
...
length(res1$quantGenes)
length(res2$quantGenes)
...
`runEISA(..., method = "published")`, so if you are happy with `runEISA` you can
